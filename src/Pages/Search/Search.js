...
const Search = () => {
  ...
  const fetchSearch = async() => {
    const {data} = await axios.get(
      `https://api.themoviedb.org/3/search/${type ? 'tv' : 'movie'}?api_key=${process.env.REACT_APP_API_KEY}&include_adult=true&include_video=false&language=en-US&query=${searchText}&page=${page}&sort_by=popularity.desc`
      );
  ...
  return (
    ...
    <TextField
    style={{flex:1}} className='searchBox' label='Search' variant='filled' 
